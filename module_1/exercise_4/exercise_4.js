...
const class_id = Math.floor(Math.random() * 4) + 1; // factor of 4 to expand the range and +1 to shift values by one
const houses = ['Gryffindor', 'Slytherin', 'Hufflepuff',  'Ravenclaw'];
let student_house;

switch (class_id) {
  case 1:
    student_house = houses[0];
    break;
  case 2:
    student_house = houses[1];
    break;
  case 3:
    student_house = houses[2];
    break;
  case 4:
    student_house = houses[3];
    break;
}

const decision_message = `${name} you are ${student_house}.`;

document.querySelector('#answer').innerHTML = decision_message;